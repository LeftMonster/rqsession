...
from rqsession.logger import logger
...
class EnhancedRequestSession(requests.Session):
    ...
    def __init__(self,
        ...
        logger.info(f"Enhanced RequestSession initialized with profile: {browser_profile}")

    def _initialize_browser_profile(self):
        """初始化浏览器配置"""
        if self.browser_profile not in self.BROWSER_PROFILES:
            logger.warning(f"Unknown browser profile: {self.browser_profile}, using default")
    ...
    def set_browser_profile(self, profile_name: str):
        ...
        self._initialize_browser_profile()
        logger.info(f"Browser profile changed from {old_profile} to {profile_name}")
    ...
    def enable_advanced_evasion_techniques(self, enabled: bool = True):
        ...
        logger.info(f"Advanced evasion techniques {'enabled' if enabled else 'disabled'}")
    ...
    def _send_advanced(self, request, **kwargs):
        ...
        try:
            ...
            logger.error(f"Rust backend request failed: {e}")
            # 降级到标准方式
            logger.info("Falling back to standard request method")
            return self._send_standard(request, **kwargs)
        except Exception as e:
    ...
    def test_fingerprint_detection(self, test_urls: Optional[List[str]] = None) -> Dict[str, Any]:
        ...
        for url in test_urls:
            try:
                logger.info(f"Testing fingerprint detection on {url}")
    ...
    def rotate_browser_profile(self):
        ...
        if new_profiles:
            ...
            logger.info(f"Browser profile rotated to: {new_profile}")
...
if __name__ == "__main__":
    ...
    with open("test_product.html", "w", encoding="utf-8") as f:
        f.write(response.text)

    # print(f"Status: {session.headers}")
    # print(f"Status: {response.status_code}")
    # print(f"Status: {response.headers}")
