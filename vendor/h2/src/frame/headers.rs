...
// Pseudo-header emission order for client requests.
// Values: 1=:method  2=:scheme  3=:authority  4=:path
// Default [1,2,3,4] matches the h2-crate native order.
// Chrome/Edge use [1,3,2,4]; Firefox [1,4,2,3]; Safari [1,2,4,3].
thread_local! {
    static PSEUDO_HEADER_ORDER: RefCell<[u8; 4]> = const { RefCell::new([1, 2, 3, 4]) };
}

/// Override the pseudo-header emission order for the current thread.
/// Call once per connection before `handshake()`.
pub fn set_pseudo_header_order(order: [u8; 4]) {
    PSEUDO_HEADER_ORDER.with(|o| *o.borrow_mut() = order);
}
...
pub struct Iter {
    ...
    /// Current position in the pseudo-header order array.
    pseudo_pos: usize,
}
...
impl Iterator for Iter {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        if let Some(ref mut pseudo) = self.pseudo {
            let order = PSEUDO_HEADER_ORDER.with(|o| *o.borrow());

            // Emit the four request pseudo-headers in the configured order.
            while self.pseudo_pos < 4 {
                let id = order[self.pseudo_pos];
                self.pseudo_pos += 1;
                match id {
                    1 => {
                        if let Some(m) = pseudo.method.take() {
                            return Some(Method(m));
                        }
                    }
                    2 => {
                        if let Some(s) = pseudo.scheme.take() {
                            return Some(Scheme(s));
                        }
                    }
                    3 => {
                        if let Some(a) = pseudo.authority.take() {
                            return Some(Authority(a));
                        }
                    }
                    4 => {
                        if let Some(p) = pseudo.path.take() {
                            return Some(Path(p));
                        }
                    }
                    _ => {}
            ...
            // Extended-CONNECT protocol pseudo-header (not in order array).
            if let Some(protocol) = pseudo.protocol.take() {
                return Some(Protocol(protocol));
            }

            // Response status pseudo-header.
