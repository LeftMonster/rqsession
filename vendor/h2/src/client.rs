...
use crate::frame::{Headers, Priority, Pseudo, Reason, Settings, StreamId};
...
pub struct Builder {
    ...
    /// PRIORITY frames to send immediately after the SETTINGS frame.
    priority_frames: Vec<Priority>,
}
...
impl Builder {
    ...
    /// Replace the entire initial SETTINGS frame with a pre-built one.
    /// Use this to control the exact settings values *and* their wire order.
    pub fn settings_frame(mut self, settings: Settings) -> Self {
        self.settings = settings;
        self
    }

    /// PRIORITY frames to send immediately after the SETTINGS frame during handshake.
    pub fn priority_frames(mut self, frames: Vec<Priority>) -> Self {
        self.priority_frames = frames;
        self
    }
    ...
    B: Buf,
{
    async fn handshake2(
        ...
        // Send browser-fingerprint PRIORITY frames immediately after SETTINGS
        for pf in &builder.priority_frames {
            codec
                .buffer(pf.clone().into())
                .expect("invalid PRIORITY frame");
        }
